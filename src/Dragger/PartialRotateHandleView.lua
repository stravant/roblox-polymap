...
local HANDLE_TICK_RADIUS_FRAC = 0.10
local HANDLE_TICK_RADIUS_LONG_FRAC = 0.30
...
function RotateHandleView:render()
	...
	if self.props.StartAngle and self.props.EndAngle then
		...
		local function createAngleDisplay(angle)
			local cframeOffset = CFrame.new(0, 0, -(radius + outerWidth) / 2)
			local cframe = self.props.HandleCFrame * CFrame.Angles(angle, 0, 0) * cframeOffset
